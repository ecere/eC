...
/*static */int AddMembers(External neededBy, OldList * declarations, Class _class, bool isMember, uint * retSize, Class topClass, bool *addedPadding)
{
   ...
   if(totalSize < maxSize && _class.type != systemClass)
   {
      ...
      {
         char sizeString[50];
         String paddingString = PrintString("__eC_padding", paddingID);
...
External _DeclareStruct(External neededBy, const char * name, bool skipNoHead, bool needDereference, bool fwdDecl)
{
   ...
   if(!classSym.registered || (classSym.registered.type == normalClass && classSym.registered.structSize && classSym.registered.base && classSym.registered.base.base))
      _DeclareStruct(neededBy, "eC::types::Instance", false, true, fwdDecl);
...
void DeclareProperty(External neededBy, Property prop, char * setName, char * getName)
{
   ...
   if((prop.Get && !symbol.externalGet) || (prop.Set && !symbol.externalSet))
   {
      if(prop._class.type == normalClass && prop._class.structSize)
         instExternal = DeclareStruct(null, "eC::types::Instance", false, true);
...
static void _DeclareType(External neededFor, Type type, bool needDereference, bool forFunctionDef, bool fwdDecl)
{
   if(inCompiler)
   {
      ...
      else if(type.kind == classType)
      {
         ...
         _DeclareStruct(neededFor, c ? c.fullName : "eC::types::Instance", c ? c.type == noHeadClass : false, needDereference && c && c.type == structClass, fwdDecl);
      }
      else if(type.kind == structType || type.kind == unionType)
      {
         Type member;
                       ...
                       bool isConversionExploration, bool warnConst)
{
   if(source && dest)
   {
      ...
      if(((source.kind == classType && dest.kind == classType) || (source.kind == subClassType && dest.kind == subClassType)) && source._class)
      {
         ...
         else
         {
                  ...
                  dest._class.registered.templateClass && source._class.registered.templateClass)
            {
               ...
               //   dest: Map<eda::Table, eda::Lookup, BT = eC::types::MapNode<KT, V, T = KT>, eda::Table, T = eC::types::MapNode<KT, V, T = KT>, eda::Table, eda::Lookup>
      ...
      // JUST ADDED THIS...
      if(source.kind == subClassType && dest.kind == classType && dest._class && !strcmp(dest._class.string, "eC::types::Class"))
         return true;
...
bool MatchTypeExpression(Expression sourceExp, Type dest, OldList conversions, bool skipUnitBla, bool warnConst)
{
   ...
   if(source)
   {
      ...
      if(dest.kind != classType && source.kind == classType && source._class && source._class.registered &&
         !strcmp(source._class.registered.fullName, "unichar" /*"eC::types::unichar"*/))
      {
         FreeType(source);
      ...
      if(dest.kind == classType)
      {
         ...
         if(_class && !strcmp(_class.fullName, "eC::types::Class") && source.kind == pointerType && source.type && source.type.kind == charType && sourceExp.type == stringExp)
         {
            OldList * specs = MkList();
            Declarator decl;
            char string[1024];
...
static Symbol ScanWithNameSpace(BinaryTree tree, const char * nameSpace, const char * name)
{
   ...
   for(symbol = (Symbol)tree.FindPrefix(nameSpace); symbol; symbol = (Symbol)((BTNode)symbol).next)
   {
      ...
      {
         // This supports e.g. matching ecere::Socket to ecereNet::Socket
         int c;
         char * namePart;
...
void ProcessExpressionType(Expression exp)
{
   ...
   switch(exp.type)
   {
      case identifierExp:
      {
         ...
         {
            exp.expType = ProcessTypeString("eC::types::Platform", true);
            break;
         }
         else if(strstr(id.string, "__eCClass") == id.string)
         {
            exp.expType = ProcessTypeString("eC::types::Class", true);
            break;
         }
               ...
               if(!prop && !member && !method)
               {
                  if(typeKind == subClassType)
                  {
                     ...
                     else
                     {
                        ...
                        classExp.expType = ProcessTypeString("eC::types::Class", false);
      ...
      case arrayExp:
      {
         ...
         if(typeString)
         {
            ...
            */
            ...
            DeclareStruct(curExternal, "eC::containers::BuiltInContainer", false, true);
...
static void ProcessInitializer(Initializer init, Type type)
{
   switch(init.type)
   {
      ...
      case listInitializer:
      {
         ...
         if(type && !type.isVector && type.kind != arrayType && type.kind != structType && type.kind != unionType && (type.kind != classType || !type._class.registered || type._class.registered.type != structClass))
         {
                     ...
                     for(spec = param.qualifiers->first; spec; spec = spec.next)
                     {
                        if(spec.type == baseSpecifier)
                        {
                           if(spec.specifier == TYPED_OBJECT)
                           {
                              ...
                              param.qualifiers = MkListOne(MkStructOrUnion(structSpecifier, MkIdentifier("__eCNameSpace__eC__types__Class"), null));
...
static void CreateFireWatcher(Property prop, Expression object, Statement stmt)
{
   ...
   if(prop.isWatchable)
   {
      ...
      ListAdd(stmt.expressions, MkExpCall(MkExpIdentifier(MkIdentifier("eC::types::eInstance_FireWatchers")), args));
      ...
      ListAdd(stmt.expressions,
         MkExpCondition(
            MkExpOp(
               ...
               MkListOne(
                  MkExpCall(MkExpIdentifier(MkIdentifier("eC::types::eInstance_FireSelfWatchers")), args)),
               ...
               for(propWatch = watches->first; propWatch; propWatch = propWatch.next)
               {
                  ...
                  if(object && object.expType && object.expType.kind == classType && object.expType._class && object.expType._class.registered)
                  {
                     ...
                     if(propWatch.deleteWatch)
                     {
                        ...
                        ListAdd(stmt.expressions, MkExpCall(MkExpIdentifier(MkIdentifier("eC::types::eInstance_WatchDestruction")), args));
                     }
                     else
                     {
                        ...
                        for(propID = propWatch.properties->first; propID; propID = propID.next)
                        {
                           ...
                           if(prop)
                           {
                              ...
                              ListAdd(stmt.expressions, MkExpCall(MkExpIdentifier(MkIdentifier("eC::types::eInstance_Watch")), args));
            ...
            if(watcher || thisClass)
            {
               if(_class)
               {
                  ...
                  if(!watches)
                  {
                     ...
                     ListAdd(args, watcher ? CopyExpression(watcher) : MkExpIdentifier(MkIdentifier("this")));
                     ListAdd(stmt.expressions, MkExpCall(MkExpIdentifier(MkIdentifier("eC::types::eInstance_StopWatching")), args));
                  }
                  else
                  {
                     for(propID = watches->first; propID; propID = propID.next)
...
void ComputeDataTypes()
{
   ...
   DeclareStruct(null, "eC::types::Class", false, true);
   DeclareStruct(null, "eC::types::Instance", false, true);
   DeclareStruct(null, "eC::types::Property", false, true);
   DeclareStruct(null, "eC::types::DataMember", false, true);
   DeclareStruct(null, "eC::types::Method", false, true);
   DeclareStruct(null, "eC::types::SerialBuffer", false, true);
   DeclareStruct(null, "eC::types::ClassTemplateArgument", false, true);
