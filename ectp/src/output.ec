   ...
   switch(stmt.type)
   {
      ...
      case returnStmt:
      {
               ...
               if(returnType.kind == templateType)
               {
                  if(returnType.templateParameter.dataTypeString)
                     decl = SpecDeclFromString(returnType.templateParameter.dataTypeString, specs,
                        MkDeclaratorIdentifier(MkIdentifier("__eCReturnVal")));
         ...
         if(inCompiler && memoryGuard)
         {
            ...
            f.Printf(" __eCNameSpace__eC__types__MemoryGuard_PopLoc();");
            if(exp && exp.expType && exp.expType.kind != voidType)
               f.Printf("return __eCReturnVal;");
            else
               f.Printf("return;");
...
static void OutputFunction(FunctionDefinition func, File f)
{
   ...
   if(func.body)
   {
      if(inCompiler && memoryGuard)
      {
         ...
         f.Printf("{\n");
         f.Printf("   __eCNameSpace__eC__types__MemoryGuard_PushLoc(\"%s:%s\");\n", name, id.string);
         outputLine += 2;
      }
