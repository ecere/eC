...
public void DumpErrors(bool display)
{
   if(globalSystem.errorBuffer && globalSystem.errorBuffer[0])
   {
      if(display)
      {
         ...
         {
            char title[1024];
            sprintf(title, "%s - Error Log", "eC Application"); // TODO: Move to Application? guiApp.appName);
...
public void SetLoggingMode(LoggingMode mode, void * where)
{
   ...
   {
#if defined(__WIN32__) && !defined(EC_BOOTSTRAP)
      uint16 * _wappName = UTF8toUTF16("eC Application" /* TODO: Move to Application? guiApp.appName*/, null);
      OutputDebugString(L"\n");
...
static DWORD REAL_ExceptionHandler(EXCEPTION_POINTERS *exception)
{
   ...
   sprintf(title, "%s - Fatal Error", "eC Application"); // TODO: Move to Application? guiApp.appName);
...
{
#if !defined(EC_BOOTSTRAP) && defined(_DEBUG)
