...
class ClassDefinition : struct;
class Instantiation : struct;
class Type : struct;

import "instance"
import "System"

/* // TODO: HTTPFile extension
#if !defined(EC_VANILLA) && !defined(EC_NONET) && !defined(EC_BOOTSTRAP)
import "HTTPFile"
#endif
*/
...
#if !defined(EC_VANILLA) && !defined(EC_NOARCHIVE) && !defined(EC_BOOTSTRAP)
...
public:
   ...
   {
   #ifdef EC_BOOTSTRAP
      fprintf(stderr, "WARNING:  File::Truncate unimplemented in bootstrapped eC library.\n");
      return false;
   #else
   #if defined(__WIN32__)
...
/* // TODO: HTTPFile extension
#if !defined(EC_VANILLA) && !defined(EC_NONET) && !defined(EC_NOCURL) // Curl version not supported yet
      ...
      else
      {
         ...
         if((d = file.dir = Dir {}))
         {
            ...
            if(d.d && (de = readdir(d.d)))
            {
               ...
               strncat(file.path, file.name, MAX_LOCATION-1);
               file.path[MAX_LOCATION-1] = 0;
      ...
      while(!Validate(extensions))
      {
         ...
         else
         {
            ...
            if(de)
            {
               ...
               strncat(path, name, MAX_LOCATION-1);
...
{
#ifndef EC_BOOTSTRAP // quick fix for now
   File f;
#if defined(__unix__) || defined(__APPLE__)
