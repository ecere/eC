...
#if defined(__linux__) && !defined(__ANDROID__) && !defined(__EMSCRIPTEN__) && defined(__x86_64__) && !defined(EC_BOOTSTRAP) && defined(__GLIBC__)
...
#define printf(...)  ((void)__android_log_print(ANDROID_LOG_VERBOSE, "ec-app", __VA_ARGS__))
#else
#define bool _bool
#define false _false
#define true _true
...
#define COM_LOAD_FUNCTION  __eCDll_Load_ecrt
#define COM_UNLOAD_FUNCTION  __eCDll_Unload_ecrt

#else

#define COM_LOAD_FUNCTION  __eCDll_Load
#define COM_UNLOAD_FUNCTION  __eCDll_Unload
                             ...
                             Module module, AccessMode declMode, AccessMode inheritanceAccess)
{
   ...
   if(inCompiler && crossBits)
   {
      ...
      if(c && c.fixed)
         fixed = true;
      else if(__thisModule.name && !strcmp(__thisModule.name, "ecrt"))
         fixed = true;
   }
   ...
   if(c - start)
   {
         ...
         else
         {
            ...
            if(base || (!baseName || !baseName[0]) || type == bitClass || type == unitClass)
            {
               ...
               if(base || !baseName || !baseName[0] ||
                  ...
                  !strcmp(baseName, "uint16"))
               {
                  if(type == normalClass && strcmp(name, "eC::types::Instance") && strcmp(name, "enum") && strcmp(name, "struct"))
                     base = eSystem_FindClass(module, "eC::types::Instance");
                  else
      ...
      if(_class)
      {
         ...
         if(crossBits)
         {
            ...
            else if(strstr(name, "eC::files::EARHeader") ||
               strstr(name, "AnchorValue") ||
               !strcmp(name, "eC::containers::CustomAVLTree") ||
               !strcmp(name, "eC::containers::Array") ||
               !strcmp(name, "ecereGUI::Window") ||
               !strcmp(name, "eC::mt::Mutex"));   // Never recompute these, they're always problematic (errors, crashes)
            else
            {
               ...
               else if(!strcmp(name, "eC::types::Class"))           size = 0; // 616
               else if(!strcmp(name, "eC::types::ClassProperty"))   size = 0; // 80
               else if(!strcmp(name, "eC::types::NameSpace"))       size = 0; // 176
               else if(!strcmp(name, "eC::files::BufferedFile"))    size = 0;
               else if(!strcmp(name, "eC::containers::BTNode"))          size = 0;
               else if(!strcmp(name, "eC::containers::StringBTNode"))    size = 0;
               else if(!strcmp(name, "eC::containers::OldList"))         size = 0; // 32
               else if(!strcmp(name, "eC::containers::Item"))            size = 0;
               else if(!strcmp(name, "eC::containers::NamedLink"))       size = 0;
               else if(!strcmp(name, "eC::containers::NamedLink64"))     size = 0;
               else if(!strcmp(name, "eC::containers::OldLink"))         size = 0;
               else if(!strcmp(name, "eC::containers::NamedItem"))       size = 0;
               else if(!strcmp(name, "eC::containers::NamedItem64"))     size = 0;
               else if(!strcmp(name, "eC::containers::BinaryTree"))      size = 0;
   ...
   // emscripten_log(EM_LOG_CONSOLE, "setting data.largest: _class: %p -- _class.data: %p\n", _class, data);
         ...
         if(base)
         {
            ...
            if(!prevBase)
            {
               if(_class.type == normalClass && strcmp(_class.name, "eC::types::Instance") && strcmp(_class.name, "enum") && strcmp(_class.name, "struct"))
                  prevBase = eSystem_FindClass(module, "eC::types::Instance");
               else
...
public dllexport void eClass_Unregister(Class _class)
{
   ...
   {
      // Unregistering templates... Used in IDE to address crash on eC classes templatized with imported modules
      OldLink templateLink;
         ...
         if(link)
         {
            ...
            if(_class && templateParams)
            {
               ...
               for(b = _class; b; b = b.base)
               {
                  ...
                  if(!strcmp(b.fullName, "eC::containers::MapNode")) isMapNode = true;
                  else if(!strcmp(b.fullName, "eC::containers::AVLNode")) isAVLNode = true;
               }
         ...
         if(curParam)
         {
            ...
            switch(curParam.type)
            {
               ...
               case expression:
               {
                  ...
                  {
                     //if(expClass.type ==
                     ((bool (*)(void *, void *, const char *))(void *)expClass._vTbl[__eCVMethodID_class_OnGetDataFromString])(expClass, &argument.expression, value);
                  }
...
static bool ConstructInstance(void * instance, Class _class, Class from, bool bindingsAlloc)
{
   ...
   #else
#if !defined(_NOMUTEX) && !defined(EC_BOOTSTRAP)
   memMutex.Wait();
#endif
      ...
      if(!name || !_class.members.FindString(name))
      {
         if(alignment)
         {
            ...
            if((force32Bits || force64Bits) &&
               name && !strcmp(_class.name, "AVLNode") && !strcmp(name, "__eCPrivateData0"))
            {
               if(force64Bits)
               {
                  type = "byte[32]";
...
static Module Module_Load(Module fromModule, const char * name, AccessMode importAccess, bool ensureCOM)
{
   ...
   if(ensureCOM && !strcmp(name, "ecrt") && module)
   {
      // REVIEW:
      name = "ecrt";
      if((!Load && !strcmp(module.name, "ecrt")) ||
         (Load && (!__thisModule || !__thisModule.name || !strcmp(__thisModule.name, "ecrt")) && Load != (void *)COM_LOAD_FUNCTION))
      {
         Module module;
         for(module = fromModule.application.allModules.first; module; module = module.next)
         {
...
static void Module_Destructor(Module module)
{
   ...
   // Unload dependencies
   {
      ...
      void * ourHandle = null;
      while((handle = module.modules.last))  // STARTING WITH LAST SO THAT ecrt IS UNLOADED LAST...
      {
...
#if defined(EC_BOOTSTRAP) || (defined(__GNUC__) && !defined(__DJGPP__) && !defined(__WIN32__) && !defined(__EMSCRIPTEN__))
#undef strlwr
#undef strupr
...
static void LoadCOM(Module module)
{
   ...
   {
      Class instanceClass = eSystem_RegisterClass(normalClass, "eC::types::Instance", null, 0, 0, null, null, module, baseSystemAccess, publicAccess);
      ...
      eClass_AddDataMember(instanceClass, "_class", "eC::types::Class", pointerSize, pointerSize, publicAccess);
   ...
   applicationClass = eSystem_RegisterClass(normalClass, "eC::types::Application", "Module", force64Bits ? (8+8+4+4 + 32 + 8 + 176) : sizeof(struct Application), 0, null, (void *)Application_Destructor, module, baseSystemAccess, publicAccess);
   if(inCompiler && force32Bits)
   {
...
public dllexport Application eCrt_Initialize(bool guiApp, int argc, char * argv[])
{
   Application app;

#ifdef EMSCRIPTEN_DEBUG
   emscripten_log(EM_LOG_CONSOLE, "eCrt_Initialize\n");
#endif
...
#ifdef _DEBUG
   // printf("Using debug eC runtime library\n");
#endif
...
public class DesignerBase // REVIEW: Can we use aggregation instead of inheritance? : Window
{
public:
...
public class ClassDesignerBase // REVIEW: Can we use aggregation instead of inheritance? : Window
{
public:
   ...
   virtual void ::CreateNew(Instance /*EditBox*/ editBox, Size clientSize, const char * name, const char * inherit);
}
