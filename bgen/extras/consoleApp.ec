...
public:
   ...
   void parseArgs()
   {
      ...
      {
         if(moduleName) isecrt = !strcmp(moduleName, "ecrt");
         ...
         if(argLibrary)
         {
            ...
            {
               case C:
                  bgen = CGen { moduleName, bindingName, defineName, isecrt };
                  break;
               case CPlusPlus:
                  bgen = CPPGen { moduleName, bindingName, defineName, isecrt };
                  break;
               case CSharp:
                  bgen = CSharpGen { moduleName, bindingName, defineName, isecrt };
                  break;
               case Java:
                  bgen = JavaGen { moduleName, bindingName, defineName, isecrt };
                  break;
               case Python:
                  bgen = PythonGen { moduleName, bindingName, defineName, isecrt };
                  break;
               default: conmsg("check"); break;
            }
            if(bgen && bgen.init())
      ...
      if(task == ArgumentSymbol::about)
      {
         ...
         PrintLn("Copyright (C) 2016-2025 Ecere Corporation");
