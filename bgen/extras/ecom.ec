...
char * copyNamespaceFullName(NameSpace * ns, Application addModuleName)
{
   ...
   if(ns)
   {
      ...
      Module m = addModuleName ? searchModulesForNamespace(addModuleName, ns, &ecrt) : null;
      NameSpace * n;
...
Module searchModulesForNamespace(Module start, NameSpace * space, bool * markEcereCOM)
{
   ...
   if(!module && start.name && !strcmp(start.name, "ecrt"))
   {
      IterNamespace ns { module = start, ecrt = true };
      while(ns.next())
      {
         if(ns.ns == space)
         {
...
{
   bool ecrt = false;
   Module mod = searchModulesForNamespace(a, c.nameSpace, &ecrt);
   return mod == m && !ecrt;
}
...
{
public:
   property Module module { set { module = value; ecrt = module.name && !strcmp(module.name, "ecrt"); } }
   bool insideFirst;
   bool siblingsOnly;
...
public:
   ...
   property bool ecrt { get { return itn.ecrt; } set { itn.ecrt = value; }  }
   IterNamespace itn { };
   BlackWhiteList list;
   Class cl;
   property Class { get { return cl; } }
