...
void cHeader(AST out, CGen g)
{
   if(!python)
   {
      ...
      if(g.lib.ecrt)
      {
         cInHeadereCuntimeFunctions(out, g);
         if(!g.preprocess)
            cInHeaderProcessSourceFile(out, g, "binding macros", ":src/c/c_header_ec_macros.src");
      }
      /*else if(g.lib.ecereGUI)
         cInHeaderEcereGUIRuntimeMacros(out, g);*/
      else if(g.lib.eda)
...
static void cInHeaderFileComment(AST out, Gen g)
{
   ...
   raw.string = CopyString(z._string); delete z;
   s = g.lib.ecrt ? CopyString("//// Core eC Library") : PrintString("//// ", g.lib.moduleName, " Module");
   out.Add(raw);
}
...
static void cInHeaderModuleName(AST out, CGen g)
{
   ASTRawString raw { }; ZString z { allocType = heap };
   z.concatx(ln);
   //if(!g.lib.ecereGUI && !g.lib.ecrt)
   {
      z.concatx(g.preproLimiter, "#if !defined(", g.lib.defineName, "_MODULE_NAME)", ln);
      z.concatx(g.preproLimiter, "#define ", g.lib.defineName, "_MODULE_NAME \"", g.lib.loadModuleName, "\"", ln);
...
static void cInHeadereCuntimeFunctions(AST out, Gen g)
{
   ASTRawString raw { }; ZString z { allocType = heap };
...
char * getFunctionNameThing(BFunction f)
{
   ...
   {
      //if(module is ecere/ecrt)
      s = strstr(f.fname, "_");
      if(s && *(++s))
         *s = (char)tolower(*s);
      //endif
      s = f.fname;
      if(*f.fname == 'e'/* && module is ecere/ecrt*/)
      {
         if(
      ...
      else/* if(module is ecere/ecrt)*/
         *s = (char)tolower(*s);
   }
   return s;
}

__attribute__((unused)) static void cInHeaderEcereGUIRuntimeMacros(AST out, Gen g)
{
   ASTRawString raw { }; ZString z { allocType = heap };
...
static void cInHeaderLibraryInitPrototype(AST out, Gen g)
{
   ASTRawString raw { }; ZString z { allocType = heap };
   if(g.lib.ecrt)
      z.concatx("extern ", !python ? "THIS_LIB_IMPORT " : "", g_.sym.application, " ", g.lib.bindingName, "_init(", g_.sym.module, " fromModule, ", python ? "eC_bool" : "C(bool)", " obsolete, ", python ? "eC_bool" : "C(bool)", " guiApp, int argc, char * argv[]);", ln);
   else
