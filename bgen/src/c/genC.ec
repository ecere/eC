...
class CGen : Gen
{
   ...
   void generate()
   {
      ...
      if(!options.headerOnly)
      {
         ...
         if(o)
         {
            ...
            sourceProcessorVars["DEP_LIBS"] = (lib.ecrt) ? CopyString("") : CopyString(
               "	$(call _L,ecrt) \\\n"
            );
   ...
   Class getClassFromType(Type type, bool reduceUnits/*, bool swapTemplateClass*/)
   {
      ...
      bool ecrtForwardDeclare = lib.ecrt &&
   ...
   void aClass(BClass c, BVariant v, bool * created)
   {
      ...
      if(!cl.templateClass)
      {
         ...
         if(!skip)
         {
            ...
            // if(g_.lib.ecereGUI && c.isWindow) skip = true;
         ...
         if(classDef)
         {
            ...
            if(typedefDI)
               ;
                  ...
                  !strcmp(c.spec, "struct __eCNameSpace__eC__types__Instance")))
               ;
            else conmsg("check");
            delete baseName;
         }
      ...
      bool instanceClass, bool * haveContent, BVariant vTop)
{
   ...
   else if(md.type == normalMethod)
   {
      if(!methodFlag || methodFlag == nonVirtualMethodImport)
      {
         ...
         if(!g.lib.ecrt || !(c.isModule && (!strcmp(md.name, "Load") || !strcmp(md.name, "Unload"))))
         {
...
{
   // if(c && !strcmp(c.cl.name, "HashMap<int64, ecere::com::Map<int, eC:types::FieldValue> >")) debugBreakpoint();
   // if(c && !strcmp(c.cl.name, "Map<int, FieldValue>")) debugBreakpoint();
   // if(c && !strcmp(c.cl.name, "Map<int, eC:types::FieldValue>")) debugBreakpoint();
