...
/*#ifdef _DEBUG
   ...
   bool onTask(int symbol)
   {
      ...
      else if(task == examples)
      {
         idnt = "  ";
         PrintLn($"Examples:");
         PrintLn(idnt, $"'bgen \"C:\\Program Files\\eC\\bin\\ecrt.dll\"'");
         PrintLn(idnt, $"'bgen ecrt'");
         PrintLn(idnt, $"'bgen dggal'");
         PrintLn(idnt, $"'bgen /usr/lib/libecrt.so'");
         return true;
      }
      else if(task == help)
      {
...
/*
AVLTree<const String> tmpcppecrtecereGUImessclass // hack
{ [
   "Window",
   "IOChannel",
   "Surface",
