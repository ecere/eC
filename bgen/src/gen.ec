...
public import "ecrt" // must keep this or compilation outside this file will fail
private:
...
private:
   ...
   virtual bool init()
   {
      ...
      expansionOrUse = expansion; // FIXME: macros are broken right now -- options.expandMacros ? expansion : use;
      //PrintLn(lang, ":");
      sym.init(this);
   ...
   bool moduleInit()
   {
      ...
      if(ec1init(lib.loadModulePath ? lib.loadModulePath : lib.loadModuleName))
      {
         ...
         conassertctx(mod != null, "?");
         {
            ...
            if(!lib.ecrt && libDeps.count == 0)
               libDeps.Insert(null, createLibrary("ecrt"));
         }
         return true;
      }
      return false;
...
public:
   ...
   void init()
   {
      ...
      ecrt = !strcmp(moduleName, "ecrt");
      eda = !strcmp(moduleName, "EDA");
      if(!bindingName)
         bindingName = CopyString(moduleName);
      if(!defineName)
...
class BFunction : struct
{
   ...
   void init(GlobalFunction fn/*, Module m*/, Map<String, String> funcRename)
   {
      ...
      //if(!(m && fn.module && fn.module != ec1HomeModule/* && !(ecrt && !fn.module)*/))
...
class BClass : struct
{
   ...
   {
      // bool ecereGUI = gen.lib.ecereGUI;
      // char * namex;
      this.cl = cl;
      ...
      /*
      if(ecereGUI && ((!py && cl.type == structClass && !strcmp(cl.name, "Size")) ||
      ...
      */
      ...
      // if(!strcmp(cl.name, "HashMap<int64, ecere::com::Map<int, eC:types::FieldValue> >")) debugBreakpoint();
...
static bool checkCompleteSpecifiedTemplateArgs(Class templateClass, Array<BClass> cTArgs)
{
   ...
   // if(!strcmp(templateClass.name, "MapNode<int, eC:types::FieldValue, T = int>")) debugBreakpoint();
   // if(!strcmp(templateClass.name, "Array<ecere::gfx::drivers::BlockEntry>")) debugBreakpoint();
   // if(!strcmp(templateClass.name, "HashMap<int64, ecere::com::Map<int, eC:types::FieldValue> >")) debugBreakpoint();
   ...
   if(complete)
   {
      ...
      */
      ...
      for(_class : lineage)
      {
         ...
         // if(!strcmp(cl.name, "HashMap") && !strcmp(templateClass.name, "HashMap<int64, ecere::com::Map<int, eC:types::FieldValue> >")) debugBreakpoint();
...
const char * bgenSymbolSwap(const char * symbol, bool reduce, bool macro)
{
   ...
   if(!cl && g_.lib.ecrt && g_.lang == CPlusPlus)
   {
