...
define eCNameSpace = "__eCNameSpace__";
...
const char * systemTypeSubst(const char * name, const char * dataTypeString)
{
   ...
   if(!((!strcmp(name, dataTypeString) && (!strcmp(name, "char") ||
         ...
         (!strcmp(name, "class") && !strcmp(dataTypeString, "struct __eCNameSpace__eC__types__Instance")) ||
         (!strcmp(name, "struct") && !strcmp(dataTypeString, "struct __eCNameSpace__eC__types__Instance")) ||
...
{
   [
      { "__eCNameSpace__eC__types__eCrt_Initialize", "eC_initApp" }
   ]
};
...
char * getMangledFunctionName(const char * functionName)
{
   ...
   name[0] = 0;
   strcat(name, eCNameSpace);
   strcat(name, functionName);
   len = strlen(name);
   for(c = strlen(eCNameSpace); c < len; c++)
   {
      if(name[c] == ':') name[c] = '_';
   }
   return name;
...
char * getSimpleDataTypeName(Type dataType, const char * dataTypeString, int indirection, bool unknownTypePermissive, bool ecrt, bool * prefix)
{
   char * name = null;
   switch(dataType.kind)
   {
      ...
      case arrayType:
      {
         ...
         name = getSimpleDataTypeName(dataType.arrayType, type, indirection, false, ecrt, null);
         break;
      }
      case classType:
      ...
      case pointerType:
      {
         ...
         name = getSimpleDataTypeName(dataType.type, type, indirection + 1, false, ecrt, null);
         break;
      }
      case functionType:
      {
         ...
         name = getSimpleDataTypeName(dataType.returnType, type, indirection, false, ecrt, null);
         break;
      }
      default:
