...
char * Gen::allocMacroSymbolNamePy(const bool noMacro, const MacroType type, const TypeInfo ti, const char * name, const char * name2, int ptr)
{
   switch(type)
   {
      case C:           return noMacro ? CopyString(name) : PrintString("eC_", name);
      ...
      case F:           return PrintString(           "eC_", name);
   ...
   void processNamespaces()
   {
      if(py)
      {
         ...
         while(itn.next())
         {
            ...
            processDefines(n);
            /*if(lib.ecrt && itn.ns->parent == null && itn.ns->name == null)
               manualTypes(n);*/
            processClasses(n);
      ...
      if(py)
      {
         ...
         while((f = itf.next()))
         {
            if(!f.skip && !f.isDllExport)
            {
               ...
               char * libName = PrintString("eC_", f.oname);
               f.nspace.addContent(v);
               f.out = out;
               // function
               tmp_merge_FuncMethOutput(out, f, null, null, paramFilter, null, name, libName, "", false, null);
               delete name;
               delete libName;
            }
         }
...
void processPyClass(PythonGen g, BClass c)
{
         ...
         /*c.isString || */c.isBool)) // (skip)
   {
      ...
      {
         out.z.concatx("(", cBase.isBool ? "Bool" : cBase.name, ")"); // c.baseSymbolName
      ...
      else if(c.isApplication)
      {
         ...
         genRegisterClassCalls(z, g, filterClasseCRTExlusions, "      ", "self");
...
// ------------------------------------------------------------------------------------------------------------------- //
            ...
            if(c.cl.type == noHeadClass)
            {
               ...
               out.z.concatx(sk, "      else:", ln,
                             sk, "         self.impl = ffi.cast(\"", c.cSymbol, " *\", lib.Instance_new(lib.class_", c.name, "))", ln);
            }
            else if(c.cl.type == structClass)
            {
               out.z.concatx(sk, "      if impl is not None:", ln,
                             sk, "         self.impl = ffi.new(\"", c.cSymbol, " *\", impl)", ln,
                             sk, "      else:", ln);
            }
            ...
            {
               if(!(c.isApplication || c.isGuiApplication || c.isContainer || (c.isWindow && g.lib.ecrt)))
               {
                  //out.z.concatx(sk, "", ln);
                  if(!cBase.isCharPtr)
                  {
...
// ------------------------------------------------------------------------------------------------------------------- //
            if(c.cl.type == bitClass)
            {
               ...
               {
                  char * name = pyGetNoConflictSymbolName(dm.name);
                  //const char * typeName = dm.dataType.kind == classType && dm.dataType._class.registered ? dm.dataType._class.registered.name : "FixTypeNameIssue"; //getSimpleDataTypeName(dm.dataType, dm.dataTypeString, 0, false, g.lib.ecrt, null);
                  BProperty p; IterConversion itc { c.cl };
                  while((p = itc.next(publicOnly)))
                  {
                     ...
                     while(itmpp.next(publicOnly))
                     {
                              ...
                              !(p.cConv && p.cConv.cl.templateClass))
                        {
                           ...
                           if(c.cl.type == structClass)
                           {
                              ...
                              out.z.concatx(sk, "            self.impl = ffi.new(\"", c.cSymbol, " *\")", ln,
...
// ------------------------------------------------------------------------------------------------------------------- //
            ...
            if(c.cl.type == structClass || c.cl.type == bitClass)
            {
               ...
               {
                  bool retype;
                  const char * typeName = itmpp.type && itmpp.type.kind == classType && itmpp.type._class.registered ? itmpp.type._class.registered.name : "FixTypeNameIssue"; //getSimpleDataTypeName(itmpp.type, dm.dataTypeString, 0, false, g.lib.ecrt, null);
                  if(itmpp.dm && itmpp.dm.type != normalMember) continue;
                           ...
                           if(itmpp.type._class.registered)
                           {
                              ...
                              switch(itmpp.type._class.registered.type)
                              {
                                 ...
                                 //case structClass:
                                    ...
                                    //      " = ffi.new(\"", itmpp.type._class.registered./*(c = .registered).cSymbol*/, " *\")",
...
// ------------------------------------------------------------------------------------------------------------------- //
            ...
            else if(c.cl.type == structClass) // c.cl.type != noHeadClass
            {
               ...
               out.z.concatx(sk, "         self.impl = ffi.new(\"", c.cSymbol, " *\"");
               if(hasUnion) // FIX #06 (16.)
               ...
               while(itmpp.next(publicOnly))
               {
                  if(itmpp.pt && itmpp.pt.Set)
                  {
                     ...
                     //if(c.cl.type == structClass)
                     {
                        ...
                        {
                           Class clBaseUnit;
                           const char * typeName = itmpp.type.kind == classType && itmpp.type._class.registered ? itmpp.type._class.registered.name : "FixTypeNameIssue"; //getSimpleDataTypeName(itmpp.type, itmpp.pt.dataTypeString, 0, false, g.lib.ecrt, null);
                           for(clBaseUnit = itmpp.type._class.registered; clBaseUnit && clBaseUnit.base.type == unitClass; clBaseUnit = clBaseUnit.base);
      ...
      // members and properties
      {
         ...
         while(itmpp.next(publicOnly))
         {
                  ...
                  (c.cl.type == noHeadClass || c.cl.type == normalClass || c.cl.type == structClass))
            {
               ...
               char * typeName = getSimpleDataTypeName(itmpptype, itmpp.dm.dataTypeString, 0, false, g.lib.ecrt, null);
            ...
            else if(itmpp.pt/* && itmpp.c == c */&& !c.isContainer) // FIX #02 // else if(itmpp.pt && !c.isContainer)
            {
               ...
               // property
               {
                  ...
                  char * typeName = getSimpleDataTypeName(p.pt.dataType, p.pt.dataTypeString, 0, false, g.lib.ecrt, null);
                  char * cSymbol = cPrintType(p.pt.dataType, false, false, true, true);
                  ...
                  else
                  {
                     ...
                     else
                     {
                        switch(p.pt.dataType.kind)
                        {
                           ...
                           case classType:
                           {
                              ...
                              {
                                 out.z.concatx("value = ", typeName, "();");
                                 out.z.concatx(" lib.", p.fpnGet, "(", selfimpl, ", ffi.cast(\"", cSymbol, "\", value.impl));");
                     ...
                     if(p.pt.Set)
                     {
                        ...
                        /*if((!mp.isProperty || pt.Set) && !(typeIsNative(itmpptype) || typeIsClassNative(itmpptype) ||
                              ...
                              //!c.cl.templateClass)
                        {
                           //const char * typeName = dm.dataType.kind == classType && dm.dataType._class.registered ? dm.dataType._class.registered.itmppname : "FixTypeNameIssue"; //getSimpleDataTypeName(dm.dataType, dm.dataTypeString, 0, false, g.lib.ecrt, null);
                           Class clBaseUnit;
                           for(clBaseUnit = itmpptype._class.registered; clBaseUnit && clBaseUnit.base.type == unitClass; clBaseUnit = clBaseUnit.base);
                           out.z.concatx(ln);
                           ...
                           switch(itmpptype.kind)
                           {
                              ...
                              case classType:
                              {
                                 if(cType.isBool)
                                    ...
                                    out.z.concatx("      lib.", p.fpnSet, "(", selfimpl, ", ffi.cast(\"", cSymbol, "\", value.impl)");
                                 else if(cType.cl.type == enumClass)
                                 {
                                    bool impl = cType.cl.base.type != systemClass;
            ...
            else if(p.cConv)
            {
               ...
               switch(p.cConv.cl.type)
               {
                  ...
                  case noHeadClass:
                     ...
                     break;
                  case structClass:
                     out.z.concatx(ln, sk, "   # def ", p.fpnGet, "(self): value = ", p.cConv.cl.name, "(); lib.", p.fpnGet, "(self.impl, ffi.cast(\"", p.cConv.cSymbol, " *\", value.impl)); return", ln);
                     break;
                  case bitClass:
                     out.z.concatx(ln, sk, "   # def ", p.fpnGet, "(self): return ", p.cConv.cl.name, "(impl = lib.", p.fpnGet, "(self.impl))", ln);
                     break;
...
//#endif 0 // temporary
      ...
      {
         out.z.concatx(ln);
         out.z.concatx(sk, c.name, ".buc = ", cBase.cl.type == unitClass ? cBase.name : c.name, ln);
      }
      // end of class
      delete lineage;
      delete classOnly;
         ...
         if(m.md.type == virtualMethod)
         {
            ...
            if(!paramsIsOnlyVoid(&m.md.dataType.params))
            {
               ...
               while(itr.next(paramFilter/*tofix: { all = true, ellipsisOn = false }*/))
               {
                  ...
                  char * type2 = strstr(type, "eC_") == type ? type + 3 : type;
                  if(thisTemplate && itr.pm == m.md.dataType.params.first)
                     out.z.concatx(", ", iname);
                  else if(itr.pm.kind == ellipsisType)
                     out.z.concatx(comma, "*ellipsisArgs(args)");
                  else
                     printArgPassing(out, comma, itr.name, type2, itr.pm, true, first, false, true);
                  delete type;
                  delete _type;
               }
            }
   ...
   else
   {
      ...
      if(!paramsIsOnlyVoid(&dataType.params))
      {
         ...
         while(itr.next(paramFilter/*tofix: { all = true, ellipsisOn = false }*/))
         {
            ...
            {
               const char * typeName = itr.pm.kind == classType && itr.pm._class.registered ? itr.pm._class.registered.name : "FixTypeNameIssue"; //getSimpleDataTypeName(itr.pm, itr.pm.dataTypeString, 0, false, g.lib.ecrt, null);
               Class clBaseUnit;
      ...
      /*else if(c && c.cl.type == structClass)
      {
         ...
         if(self)
            out.z.concatx("ffi.cast(\"", c.cSymbol, c.cl.type == structClass ? " *" : "", "\", self.impl)");
      }*/
      else
      {
         // FIX #05
         ...
         if(self && c && c.cl.type == structClass)
            out.z.concatx("ffi.cast(\"", c.cSymbol, " *", "\", self.impl)"); // todo: move this case (structClass) to selfimpl initialization
         else if(self)
         ...
         while(itr.next(paramFilter/*tofix: { all = true, ellipsisOn = false }*/))
         {
            ...
            if(itr.isVoidPtrReturn)
            {
               ...
               if(cPrev && cPrev.isClass)
               {
                  ...
                  out.z.concatx(sk, spaces(ind+3, 0), "i = ffi.cast(\"eC_Instance\", ", itr.name, "[0])", ln);
      ...
      if(multireturn > 0)
      {
         ...
         while(itr.next(paramFilter/*tofix: { all = true, ellipsisOn = false }*/))
         {
            if(itr.isReturnValue)
            {
               ...
               {
                  const char * typeName = itr.pm.type.kind == classType && itr.pm.type._class.registered ? itr.pm.type._class.registered.name : "FixTypeNameIssue"; //getSimpleDataTypeName(itr.pm, dm.dataTypeString, 0, false, g.lib.ecrt, null);
         ...
         else
         {
                  ...
                  (param._class.registered.type == noHeadClass || param._class.registered.type == structClass))
            {
               if(param._class.registered.type == noHeadClass)
                  out.z.concatx(comma, "ffi.cast(\"struct ", c.cSymbol, " *\", ", name/*, c && c.cl.type == normalClass ? ".impl" : ""*/, ")");
               else
                  out.z.concatx(comma, "ffi.cast(\"", c.cSymbol, " *\", ", name/*, c && c.cl.type == normalClass ? ".impl" : ""*/, ")");
            }
            else
            {
               //bool ptr = true;
               //if(c.cl.type == structClass)
               //   out.z.concatx(comma, "ffi.cast(\"", c.cSymbol, " *", "\", ", name, ".impl)");
               //else
...
bool classHasContent(PythonGen g, BClass c)
{
   ...
   {
      if(/*c.isString || (*/c.isWindow && g.lib.ecrt)//))
         return false;
      return true;
   }
...
static void generatePY(File out, PythonGen g)
{
   for(libDep : g.libDeps)
      out.PrintLn("from ", (libDep.ecrt /*|| libDep.ecere || libDep.eda*/) ? "eCSDK" : libDep.bindingName, ".", libDep.bindingName, " import *");
...
bool filterClasseCRTExlusions(BClass c) { return !c.isCharPtr && !c.isWindow; }
...
// out.PrintLn("");
   ...
   out.PrintLn("print()");
   //out.PrintLn("t1 = path.join(rel, '../c', 'ecrt.c')");
   //out.PrintLn("print(\"path.join(rel, '../c', 'ecrt.c') is \", t1)");
#endif // 0
   for(libDep : g.libDeps)
   ...
   out.PrintLn("   srcs.append(path.join(cpath, 'ecrt.c'))");
   if(!g.lib.ecrt)
...
#define _EMBEDDED_C
   ...
   //else if(g.lib.ecere)
   //   out.PrintLn("               sources=['../c/ecrt.c', '../c/", g.lib.bindingName, ".c'],");
   else
      out.PrintLn("               sources=['../c/ecrt.c', '../c/", g.lib.bindingName, ".c'],");
#endif
#endif
...
#endif
#endif // 0
   out.Print("               extra_link_args=[\"-Wl,-rpath,$ORIGIN/lib,-rpath,$ORIGIN/eCSDK/lib\","); //,-rpath,$ORIGIN
   if(g.libDeps.count)
   {
      bool first = true;
      //bool ecere = false;
   ...
   //out.PrintLn("               library_dirs=[path.join(owd, rel, '../../obj/win32/bin'),'C:/Program Files/eCSDK/bin'"/*, g.libDeps.count ? ", '.'" : ""*/, "])"); // todo
