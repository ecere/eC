...
{ [
   { "ecrt", "Window" },                    // Dependency on Window
   { "ecrt", "ClassDesignerBase" },         // Dependency on Window
   { "ecrt", "DesignerBase" },              // Dependency on Window
...
class CPPGen : CGen
{
   ...
   bool init()
   {
      ...
      // FIXME: macros are broken right now -- if(options.expandMacros)
         macroModeBits.all = expansion;
      return result;
   }
   ...
   void prepareClasses(BNamespace n)
   {
      ...
      while((c = itc.next(all)))
      {
         ...
         if(/*g_.lib.ecrt && */skipClasses.Find({ g_.lib.bindingName, c.cl.name }))
            skip = true;
   ...
   void processNamespaces()
   {
      ...
      while(ns.next())
      {
         ...
         processDefines(n);
         //if(lib.ecrt && ns.ns->parent == null && ns.ns->name == null)
         //   manualTypes(n);
         processClasses(n);
   ...
   void generate()
   {
      ...
      if(!options.headerOnly)
      {
         ...
         if(f)
         {
            ...
            sourceProcessorVars["DEP_FILE_LISTS"] = true /*!lib.ecrt*/ ? CopyString("") : CopyString(
               "_DEP_OBJECTS = \\\n"
               "	$(if $(_embedded_c),$(OBJ)ecrt.c$(O),) \\\n"
               "	$(OBJ)ecrt$(O)\n"
               "\n"
               "_DEP_SOURCES = \\\n"
               "	$(if $(_embedded_c),../c/ecrt.c,) \\\n"
               "	ecrt.cpp\n"
               "\n"
            );
            sourceProcessorVars["DEP_RULES"] = true /*!lib.ecrt*/ ? CopyString("") : CopyString(
               "ifdef _embedded_c\n"
               "$(OBJ)ecrt.c$(O): ../c/ecrt.c\n"
            ...
            sourceProcessorVars["DEP_LIBS"] = CopyString((lib.ecrt) ? "" :
               "	$(call _L,ecrt) \\\n"
               // "	$(if $(_embedded_c),,$(call _L,ecrt_c)) \\\n"
               "	$(call _L,ecrt_cpp) \\\n" // line continuation for some makefiles have stuff following this? :S
            );

            // sourceProcessorVars["SPECIFIC_FLAGS"] = CopyString(!lib.ecrt ? "" : "PRJ_CFLAGS += -DECRT_ONLY\n\n");
...
static void generateHPP(CPPGen g, File f)
{
   ...
   // if(g.lib.ecrt) prototypeHackClasses(g, f);
...
bool typedefClasses(CPPGen g, File f)
{
   ...
   while((c = itacl.next(all)))
   {
      if(c.cl.type == enumClass)
      {
         BClass cBase = c.cl.base;
         bool skip = c.isBool || c.isByte || c.isCharPtr || c.isUnInt || c.isUnichar || c.is_class || c.isString ||
            (g.lib.ecrt && skipClasses.Find({ g_.lib.bindingName, c.cl.name }))/* temporary: */ || !cBase || !cBase.isBool; // temporary
         if(!skip)
         {
...
bool prototypeClasses(CPPGen g, File f)
{
   ...
   while((c = itacl.next(all)))
   {
      bool skip = c.isBool || c.isByte || c.isCharPtr || c.isUnInt || c.isUnichar || c.is_class || c.isString;
      bool template = hasTemplateClass(c.cl);
      if(g.lib.ecrt && (c.isSurface || /*c.isIOChannel || */c.isWindow || c.isDataBox))
         ; //locprintx("if(g.lib.ecrt && (c.isSurface || /*c.isIOChannel || */c.isWindow || c.isDataBox))");
      if(g.lib.ecrt && skipClasses.Find({ g_.lib.bindingName, c.cl.name }))
         skip = true;
      if(!skip && !template)
      {
         bool hasOutput;
...
static void generateCPP(CPPGen g, File f)
{
   ...
   */
   ...
   if(g.lib.ecrt) // hardcoded
   {
      f.Print(ln);
      f.PrintLn(genloc__, "// Instance methods depending on libecrt");
      f.Print(genloc__, "void Instance::class_registration(CPPClass & _cpp_class)\n");
      // TONOMACRO
      if(true) // FIXME: macros are broken right now -- g.options.expandMacros)
      {
...
static void processCppClass(CPPGen g, BClass c, BClass cRealBase)
{
   ...
   if(!skip)
   {
      ...
      if((c.cl.type == normalClass || c.cl.type == noHeadClass || c.cl.type == structClass || c.cl.type == bitClass || c.cl.type == unitClass || c.cl.type == enumClass) &&
            !(g.lib.ecrt && (c.isSurface || /*c.isIOChannel || */c.isWindow || c.isDataBox)))
      {
      ...
      // definitions:
      // if(!(g.lib.ecereGUI && c.isDataDisplayFlags))
      {
         ...
         if(c.cl.type == normalClass && !(g.lib.ecrt && (c.isSurface || /*c.isIOChannel || */c.isWindow || c.isDataBox)))
         {
         ...
         {
            o.z.concatx(ln);
            o.z.concatx(genloc__, "int ecrt_cpp_init(const Module & module);", ln); // todo? back to void returning?
            ...
            else if(c.cl.type == normalClass)
            {
               ...
               // if(mode != expansion)
               {
                  if(c.isApplication)
                  {
                     ...
                     o.z.concatx(genloc__, "      ecrt_cpp_init(*this);", ln);
                     /*
                     o.z.concatx(genloc__, "#if !defined(ECRT_ONLY)", ln);
...
static void processDataMembers(CPPGen g, BClass c, BVariant v, const char * cn, bool prototype, BOutput o)
{
   ...
   for(e : c.cumulationLineage)
   {
      ...
      while((dm = dat.next(publicOnly)))
      {
         ...
         if(mn && strstr(mn, "__eCPrivateData") == mn) debugBreakpoint();
   ...
   i.e.
      ...
      double array[16]; -> double * array;
         */
         ...
         if(eClass_FindProperty(c2.cl, mn, c2.cl.module) || strstr(mn, "__eCPrivateData")) continue;
   ...
   if(cXT && cXT.cl.templateArgs)
   {
      ...
      else
      {
         const char * typeString = ti.type._class.string; // strptrNoNamespace(ti.type._class.string);
              if(!strcmp(typeString, "eC::containers::Container<T>"))                      x = CopyString("TContainer<TP_T>");
         else if(!strcmp(typeString, "eC::containers::Iterator<T>"))                       x = CopyString("TIterator<TP_T>");
         else if(!strcmp(typeString, "eC::containers::Container<T, IT>"))                  x = CopyString("TContainer<TP_T, TP_IT>");
         else if(!strcmp(typeString, "eC::containers::HashMap<KT, VT>"))                   x = CopyString("THashMap<TP_KT, TP_VT>");
         else if(!strcmp(typeString, "eC::containers::Map<KT, V>"))                        x = CopyString("TMap<TP_KT, TP_V>");

         // else if(!strcmp(typeString, "eC::containers::Array<ecere::gfx3D::MeshPart>"))     x = CopyString("TArray<MeshPart>");
         // else if(!strcmp(typeString, "eC::containers::Array<String>"))                     x = CopyString("TArray<String>");
         // else if(!strcmp(typeString, "eC::containers::Array<ecere::gui::Window>"))         x = CopyString("TArray<Window>");
         /*
         else if(!strcmp(typeString, "eC::containers::List<ecere::net::CallAck>"))         x = CopyString("TList<CallAck>");
         else if(!strcmp(typeString, "eC::containers::List<ecere::net::VirtualCallAck>"))  x = CopyString("TList<TVirtualCallAck>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<const String>"))               x = CopyString("TArray<constString>");
         // else if(!strcmp(typeString, "eC::containers::Array<eda::IdFilter>"))              x = CopyString("TArray<IdFilter>");
         // else if(!strcmp(typeString, "eC::containers::Array<eda::Grouping>"))              x = CopyString("TArray<Grouping>");
         // else if(!strcmp(typeString, "eC::containers::Array<eC::containers::Class>"))          x = CopyString("TArray<Class>");
         // else if(!strcmp(typeString, "eC::containers::Array<eda::SQLiteSearchField>"))     x = CopyString("TArray<SQLiteSearchField>");
         // else if(!strcmp(typeString, "eC::containers::Array<eda::StringSearchField>"))     x = CopyString("TArray<StringSearchField>");
         /*
         else if(!strcmp(typeString, "eC::containers::Container<eda::Field>"))             x = CopyString("TContainer<Field>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<eda::ListField>"))             x = CopyString("TArray<ListField>");
         // else if(!strcmp(typeString, "eC::containers::Array<eda::StringSearchTable>"))     x = CopyString("TArray<StringSearchTable>");
         // else if(!strcmp(typeString, "eC::containers::Array<eda::SQLiteSearchTable>"))     x = CopyString("TArray<SQLiteSearchTable>");
         // else if(!strcmp(typeString, "eC::containers::Array<eda::LookupEditor>"))          x = CopyString("TArray<LookupEditor>");
         // else if(!strcmp(typeString, "eC::containers::Array<RecordField>"))                x = CopyString("TArray<RecordField>");
         // else if(!strcmp(typeString, "eC::containers::Array<double>"))                     x = CopyString("TArray<double>");
         /*
         else if(!strcmp(typeString, "CMSSList<CMSSExpression>"))                      x = CopyString("TCMSSList<CMSSExpression>");
         else if(!strcmp(typeString, "eC::containers::List<CMSSNode>"))                    x = CopyString("TList<CMSSNode>");
         else if(!strcmp(typeString, "eC::containers::List<CMSSIdentifier>"))              x = CopyString("TList<CMSSIdentifier>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<ValueColor>"))                 x = CopyString("TArray<ValueColor>");
         // else if(!strcmp(typeString, "eC::containers::Array<ValueOpacity>)"))              x = CopyString("TArray<ValueOpacity>)");
         // else if(!strcmp(typeString, "eC::containers::Array<GraphicalElement>"))           x = CopyString("TArray<GraphicalElement>");
         // else if(!strcmp(typeString, "eC::containers::Array<ValueOpacity>"))               x = CopyString("TArray<ValueOpacity>");
         // else if(!strcmp(typeString, "eC::containers::Array<ecere::gfx::ColorKey>"))       x = CopyString("TArray<ColorKey>");
         // else if(!strcmp(typeString, "eC::containers::Array<GeoData>"))                    x = CopyString("TArray<GeoData>");
         /*
         else if(!strcmp(typeString, "eC::containers::Map<String, eC:types::FieldValue>"))      x = CopyString("TMap<String, FieldValue>");
         else if(!strcmp(typeString, "eC::containers::Map<String, GeoJSONValue>"))         x = CopyString("TMap<String, GeoJSONValue>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<GeoJSONFeature>"))             x = CopyString("TArray<GeoJSONFeature>");
         // else if(!strcmp(typeString, "eC::containers::Array<GeoJSONSegment>"))             x = CopyString("TArray<GeoJSONSegment>");
         /*
         else if(!strcmp(typeString, "eC::containers::List<GeoLayer>"))                    x = CopyString("TList<GeoLayer>");
         else if(!strcmp(typeString, "eC::containers::Map<String, CMSSStyleSheet>"))       x = CopyString("TMap<String, CMSSStyleSheet>");
         else if(!strcmp(typeString, "eC::containers::List<Presentation>"))                x = CopyString("TList<Presentation>");
         else if(!strcmp(typeString, "eC::containers::Map<ecere::gfx::Color, eC::containers::Array<uint64> >")) x = CopyString("TMap<Color, TArray<uint64>>");
         else if(!strcmp(typeString, "eC::containers::Container<LineString>"))             x = CopyString("TContainer<LineString>");
         else if(!strcmp(typeString, "eC::containers::Container<GeoPoint>"))               x = CopyString("TContainer<GeoPoint>");
         else if(!strcmp(typeString, "eC::containers::Map<String, MBGLSpriteSymbol>"))     x = CopyString("TMap<String, MBGLSpriteSymbol>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<int64>"))                      x = CopyString("TArray<int64>");
         /*
         else if(!strcmp(typeString, "eC::containers::List<GeoExtent>"))                   x = CopyString("TList<GeoExtent>");
         else if(!strcmp(typeString, "eC::containers::Map<String, MapboxGLSourceData>"))   x = CopyString("TMap<String, MapboxGLSourceData>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<MBGLLayersJSONData>"))         x = CopyString("TArray<MBGLLayersJSONData>");
         /*
         else if(!strcmp(typeString, "eC::containers::Container<GraphicalElement>"))       x = CopyString("TContainer<GraphicalElement>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<StackFrame>"))                 x = CopyString("TArray<StackFrame>");
         /*
         else if(!strcmp(typeString, "eC::containers::Container<eC::types::Pointf>"))     x = CopyString("TContainer<Pointf>");
         else if(!strcmp(typeString, "eC::containers::Container<ecere::gfx3D::Vector3Df>")) x = CopyString("TContainer<Vector3Df>");
         else if(!strcmp(typeString, "eC::containers::Container<PolygonContour>"))         x = CopyString("TContainer<PolygonContour>");
         else if(!strcmp(typeString, "eC::containers::Container<StartEndPair>"))           x = CopyString("TContainer<StartEndPair>");
         else if(!strcmp(typeString, "eC::containers::Container<Polygon>"))                x = CopyString("TContainer<Polygon>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<ProcessInvocation>"))          x = CopyString("TArray<ProcessInvocation>");
         // else if(!strcmp(typeString, "eC::containers::Array<int>"))                        x = CopyString("TArray<int>");
         // else if(!strcmp(typeString, "eC::containers::Array<TSRow>"))                      x = CopyString("TArray<TSRow>");
         // else if(!strcmp(typeString, "eC::containers::Array<TSZoomLevel>"))                x = CopyString("TArray<TSZoomLevel>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3IdentifierAndLinks>"))     x = CopyString("TArray<WFS3IdentifierAndLinks>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3Link>"))                   x = CopyString("TArray<WFS3Link>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3Collection>"))             x = CopyString("TArray<WFS3Collection>");
         // else if(!strcmp(typeString, "eC::containers::Array<eC::containers::Array<String> >")) x = CopyString("TArray<TArray<String>>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3Queryable>"))              x = CopyString("TArray<WFS3Queryable>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3VariableWidth>"))          x = CopyString("TArray<WFS3VariableWidth>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3TileMatrixSetLimit>"))     x = CopyString("TArray<WFS3TileMatrixSetLimit>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3TileMatrixSetLink>"))      x = CopyString("TArray<WFS3TileMatrixSetLink>");
         // else if(!strcmp(typeString, "eC::containers::Array<WFS3TileMatrix>"))             x = CopyString("TArray<WFS3TileMatrix>");
         /*
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFAccessor>"))         x = CopyString("TMap<String, glTFAccessor>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFAnimation>"))        x = CopyString("TMap<String, glTFAnimation>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFBufferView>"))       x = CopyString("TMap<String, glTFBufferView>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFBuffer>"))           x = CopyString("TMap<String, glTFBuffer>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFImage>"))            x = CopyString("TMap<String, glTFImage>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFMaterial>"))         x = CopyString("TMap<String, glTFMaterial>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFMesh>"))             x = CopyString("TMap<String, glTFMesh>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFNode>"))             x = CopyString("TMap<String, glTFNode>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFProgram>"))          x = CopyString("TMap<String, glTFProgram>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFSampler>"))          x = CopyString("TMap<String, glTFSampler>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFScene>"))            x = CopyString("TMap<String, glTFScene>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFShader>"))           x = CopyString("TMap<String, glTFShader>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFSkin>"))             x = CopyString("TMap<String, glTFSkin>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFTechnique>"))        x = CopyString("TMap<String, glTFTechnique>");
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFTexture>"))          x = CopyString("TMap<String, glTFTexture>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<glTFAnimationChannel>"))       x = CopyString("TArray<glTFAnimationChannel>");
         /*
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFAnimationSampler>")) x = CopyString("TMap<String, glTFAnimationSampler>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<glTFPrimitive>"))              x = CopyString("TArray<glTFPrimitive>");
         /*
         else if(!strcmp(typeString, "eC::containers::Map<String, String>"))               x = CopyString("TMap<String, String>");
         */
         // else if(!strcmp(typeString, "eC::containers::Array<uint>"))                       x = CopyString("TArray<uint>");
         // else if(!strcmp(typeString, "eC::containers::Array<bool>"))                       x = CopyString("TArray<bool>");
         /*
         else if(!strcmp(typeString, "eC::containers::Map<String, glTFTechniqueParameter>")) x = CopyString("TMap<String, glTFTechniqueParameter>");
         else if(!strcmp(typeString, "")) x = CopyString("");
         */
         else if(!strcmp(typeString, "eC::containers::LinkElement<thisclass>")) ;
         else
         {
            PrintLn("template type: ", cXT.cl.name, " -> ", x ? x : "????");
            debugBreakpoint();
...
/*
   ...
   */
   ...
   if(!newTemplateType && ti.type.kind == classType && ti.type._class && strchr(ti.type._class.string, '<') && strchr(ti.type._class.string, '>') )
   {
      ...
      else if(!strcmp(typeString, "eC::containers::Container<T>"))                      x = CopyString("TContainer<TP_T>"), b = true;
      else if(!strcmp(typeString, "eC::containers::Iterator<T>"))                       x = CopyString("TIterator<TP_T>"), b = true;
      else if(!strcmp(typeString, "eC::containers::Container<T, IT>"))                  x = CopyString("TContainer<TP_T, TP_IT>"), b = true;
      else if(!strcmp(typeString, "eC::containers::HashMap<KT, VT>"))                   x = CopyString("THashMap<TP_KT, TP_VT>"), b = true;
      else if(!strcmp(typeString, "eC::containers::Map<KT, V>"))                        x = CopyString("TMap<TP_KT, TP_V>"), b = true;
      ...
      else if(!strcmp(typeString, "eC::containers::LinkElement<thisclass>")) x = CopyString("TLinkElement<thisclass>");
      else if(!strcmp(typeString, "")) x = CopyString("");
   ...
   /*
   if(ti.type.kind == classType && ti.type._class && !strcmp(ti.type._class.string, "eC::containers::Iterator<T>"))
   {
      // debugBreakpoint();
      delete tn;
   ...
   */
   ...
   // ti.type.kind != arrayType)         // todo (broken stuff that's not generated)
   {
      if(isProp)
      {
         different = (eClass_FindDataMember(c.cl, mn, c.cl.module, null, null) || strstr(mn, "__eCPrivateData"));
         if(hasGet || different)
            genGet = true;
         genSet = hasSet;
      }
...
ClassType cppGetClassInfoFromType(Type type, bool hackTemplates, Class * clRegRet, BClass * cParamRet, bool * hackRet, bool * isString, bool * isConst)
{
   ...
   if(type.kind == classType && type._class)
   {
      ...
      /*
      if(!clReg && name && g_.lib.ecrt && tmpcppececeremessclass.Find(name))
      ...
      */
      ...
      hack = false; /*g_.lib.ecrt && (ct == normalClass || ct == noHeadClass) &&
         ...
         // if(mode == regMethodParamList)
         {
            ...
            for(param = firstParam ? firstParam : templateParam ? templateParam : t.params.first; param; param = nextParam)
            {
               if(param.kind != voidType)
               {
                  ...
                  if(g_.lib.ecrt && param.kind == classType && !clReg && !strcmp(param._class.string, "Alignment")) // hack
                     ct = enumClass;
      ...
      case expansion:
         ...
         else
         {
            ...
            if(!strcmp(c, "Application") || !strcmp(c, "GuiApplication"))
               o.concatx(genloc__, indents(ind), "inline ", c, "() : ", c, "(ecrt_init_CALL(__LINK_ECERE__)) { }", ln);
            else
            {
         ...
         while((m = itm.next(publicVirtual)))
         {
            ...
            //    itm.md.dataType.thisClass.type = processTypeStringOk(itm.md.dataType.thisClass.string);
            {
               ...
               argsInfo = { type = t, m = m, md = m.md, cl = c.cl, c = c };
               {
                  ...
                  cppMacroVirtualMethod(g, o, submode, prototype, template, structOrNohead, ind + 1 /* // FIXME: macros are broken right now -- g.options.expandMacros*/ && !prototype ? 0 : 1,
      ...
      void * unused)
{
   ...
   switch(smod)
   {
      ...
      case expansion:
         ...
         if(true /*g.options.expandMacros // FIXME: macros are broken right now -- */ && !structOrNohead) // temp: this is removed now replace by new REG_* macros and C++ crazy template stuff
   ...
   if(cl.templateArgs)
   {
      ...
      if(!c.isClassTemplatable/*c.completeTemplate*/)
      {
         ...
         if(cSymbolTArgs.count && c.completeTemplate)
         {
            ...
            else if(!strcmp(c.cl.name, "Array<eC::containers::Class>") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "C(Class)") && !strcmp(cSymbolTArgs[1], "int") && !strcmp(cSymbolTArgs[2], "C(Class)")) ; // else if(!strcmp(c.cl.name, "Array<eC::containers::Class>") && cSymbolTArgs.count == 1 && !strcmp(cSymbolTArgs[0], "C(Class)")) ;
            ...
            else if(!strcmp(c.cl.name, "Array<eC:types::FieldValue>") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "C(FieldValue)") && !strcmp(cSymbolTArgs[1], "int") && !strcmp(cSymbolTArgs[2], "C(FieldValue)")) ; // else if(!strcmp(c.cl.name, "Array<eC:types::FieldValue>") && cSymbolTArgs.count == 1 && !strcmp(cSymbolTArgs[0], "C(FieldValue)")) ;
            ...
            // else if(!strcmp(c.cl.name, "CustomAVLTree<eC::containers::MapNode<MT, V>, I = MT, D = V, KT = MT>") && cSymbolTArgs.count == 2 && !strcmp(cSymbolTArgs[0], "T(MapNode, MT, V)") && !strcmp(cSymbolTArgs[1], "T(MapNode, MT, V)")) ;
            ...
            // else if(!strcmp(c.cl.name, "LinkList<eC::containers::Link, T = LLT, D = LLT>") && cSymbolTArgs.count == 1 && !strcmp(cSymbolTArgs[0], "T(AVLNode, KT)")) ;
            ...
            else if(!strcmp(c.cl.name, "MapNode<int, eC:types::FieldValue, T = int>") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "int") && !strcmp(cSymbolTArgs[1], "int") && !strcmp(cSymbolTArgs[2], "C(FieldValue)")) ; // else if(!strcmp(c.cl.name, "MapNode<int, eC:types::FieldValue, T = int>") && cSymbolTArgs.count == 2 && !strcmp(cSymbolTArgs[0], "int") && !strcmp(cSymbolTArgs[1], "C(FieldValue)")) ;
            else if(!strcmp(c.cl.name, "Map<int, eC:types::FieldValue>") && cSymbolTArgs.count == 2 && !strcmp(cSymbolTArgs[0], "int") && !strcmp(cSymbolTArgs[1], "C(FieldValue)")) ;
            else if(!strcmp(c.cl.name, "HashMap<int64, eC::containers::Map<int, eC:types::FieldValue> >") && cSymbolTArgs.count == 2 && !strcmp(cSymbolTArgs[0], "int64") && !strcmp(cSymbolTArgs[1], "T(Map, int, FieldValue)")) ;
            else if(!strcmp(c.cl.name, "MapNode<eC::containers::Array<int>, eC::containers::Array<int64>, T = eC::containers::Array<int> >") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "T(Array, int)") && !strcmp(cSymbolTArgs[1], "T(Array, int)") && !strcmp(cSymbolTArgs[2], "T(Array, int64)")) ; // else if(!strcmp(c.cl.name, "MapNode<eC::containers::Array<int>, eC::containers::Array<int64>, T = eC::containers::Array<int> >") && cSymbolTArgs.count == 2 && !strcmp(cSymbolTArgs[0], "T(Array, int)") && !strcmp(cSymbolTArgs[1], "T(Array, int)")) ;
            else if(!strcmp(c.cl.name, "Map<eC::containers::Array<int>, eC::containers::Array<int64> >") && cSymbolTArgs.count == 2 && !strcmp(cSymbolTArgs[0], "T(Array, int)") && !strcmp(cSymbolTArgs[1], "T(Array, int64)")) ; // else if(!strcmp(c.cl.name, "Map<eC::containers::Array<int>, eC::containers::Array<int64> >") && cSymbolTArgs.count == 4 && !strcmp(cSymbolTArgs[0], "T(MapNode, Array<int>, Array<int64>, T = Array<int> )") && !strcmp(cSymbolTArgs[1], "T(MapNode, Array<int>, Array<int64>, T = Array<int> )") && !strcmp(cSymbolTArgs[2], "T(Array, int)") && !strcmp(cSymbolTArgs[3], "T(Array, int64)")) ;
            ...
            else if(!strcmp(c.cl.name, "MapNode<String, eC:types::FieldValue, T = String>") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "C(String)") && !strcmp(cSymbolTArgs[1], "C(String)") && !strcmp(cSymbolTArgs[2], "C(FieldValue)")) ;
            else if(!strcmp(c.cl.name, "Map<String, eC:types::FieldValue>") && cSymbolTArgs.count == 2 && !strcmp(cSymbolTArgs[0], "C(String)") && !strcmp(cSymbolTArgs[1], "C(FieldValue)")) ; // else if(!strcmp(c.cl.name, "Map<String, eC:types::FieldValue>") && cSymbolTArgs.count == 7 && !strcmp(cSymbolTArgs[0], "T(MapNode, String, FieldValue, T = String)") && !strcmp(cSymbolTArgs[1], "C(String)") && !strcmp(cSymbolTArgs[2], "C(FieldValue)") && !strcmp(cSymbolTArgs[3], "T(MapNode, String, FieldValue, T = String)") && !strcmp(cSymbolTArgs[4], "C(String)") && !strcmp(cSymbolTArgs[5], "C(String)") && !strcmp(cSymbolTArgs[6], "C(FieldValue)")) ;
            ...
            else if(!strcmp(c.cl.name, "MapNode<ecere::gfx::Color, eC::containers::Array<uint64>, T = ecere::gfx::Color>") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "C(Color)") && !strcmp(cSymbolTArgs[1], "C(Color)") && !strcmp(cSymbolTArgs[2], "T(Array, uint64)")) ;
            else if(!strcmp(c.cl.name, "Map<ecere::gfx::Color, eC::containers::Array<uint64> >") && cSymbolTArgs.count == 7 && !strcmp(cSymbolTArgs[0], "T(MapNode, Color, Array<uint64>, T = Color)") && !strcmp(cSymbolTArgs[1], "C(Color)") && !strcmp(cSymbolTArgs[2], "T(Array, uint64)") && !strcmp(cSymbolTArgs[3], "T(MapNode, Color, Array<uint64>, T = Color)") && !strcmp(cSymbolTArgs[4], "C(Color)") && !strcmp(cSymbolTArgs[5], "C(Color)") && !strcmp(cSymbolTArgs[6], "T(Array, uint64)")) ;
            ...
            else if(!strcmp(c.cl.name, "Container<eC::types::Pointf>") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "C(Pointf)") && !strcmp(cSymbolTArgs[1], "int") && !strcmp(cSymbolTArgs[2], "C(Pointf)")) ;
            ...
            else if(!strcmp(c.cl.name, "Array<eC::containers::Array<String> >") && cSymbolTArgs.count == 3 && !strcmp(cSymbolTArgs[0], "T(Array, String)") && !strcmp(cSymbolTArgs[1], "int") && !strcmp(cSymbolTArgs[2], "T(Array, String)")) ;
         ...
         else
         {
            if(c.numTemplateArgsInName)
            {
               ...
               else if(!strcmp(c.cl.name, "CustomAVLTree<BT = eC::containers::AVLNode<AT>, KT = AT, T = AT, D = AT>")) ;
               ...
               else if(!strcmp(c.cl.name, "CustomAVLTree<eC::containers::MapNode<MT, V>, I = MT, D = V, KT = MT>")) ;
               else if(!strcmp(c.cl.name, "Iterator<V, IT = KT>")) ;
               else if(!strcmp(c.cl.name, "Iterator<VT, IT = KT>")) ;
               else if(!strcmp(c.cl.name, "LinkList<eC::containers::Link, T = LLT, D = LLT>")) ;
